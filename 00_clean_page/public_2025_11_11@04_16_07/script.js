...
function createPanelManager() {
  ...
  overlay.hidden = true;
  overlay.innerHTML = `
    <div class="panel-overlay__backdrop" data-panel-close></div>
    <div class="panel-overlay__stage" role="dialog" aria-modal="true" tabindex="-1">
      <div class="panel-overlay__content" data-panel-content></div>
    </div>
  `;

  appContainer.appendChild(overlay);

  const panelContentHost = overlay.querySelector("[data-panel-content]");
  ...
  async function openPanel({ id, variant, title }) {
    ...
    panelContentHost.appendChild(fallbackRoot);
    ...
    try {
      ...
      panelContentHost.innerHTML = "";
      panelContentHost.appendChild(fragment);

      const panelRoot =
        panelContentHost.querySelector("[data-panel-shell]") || panelContentHost.firstElementChild;
      if (panelRoot) {
