(function () {
  ...
  window.PanelModules["design-product-preview"] = function initDesignProductPreview(root) {
    ...
    const viewToggleButtons = root.querySelectorAll("[data-view-toggle-btn]");
    const frameToggleBtn = root.querySelector("[data-frame-toggle-btn]");
    const designToggleBtn = root.querySelector("[data-design-toggle-btn]");
    const previewImg = root.querySelector("[data-preview-img]");
    const previewStage = root.querySelector(".product-preview__stage");
    const overlaySvg = root.querySelector(".product-preview__canvas-overlay");
    const overlayRect = root.querySelector(".product-preview__canvas-rect");
    ...
    const syncVisibilityButtons = () => {
      if (frameToggleBtn) {
        frameToggleBtn.classList.toggle("is-active", !!state.showFrame);
        frameToggleBtn.setAttribute("aria-pressed", String(!!state.showFrame));
      }
      if (designToggleBtn) {
        designToggleBtn.classList.toggle("is-active", !!state.showDesign);
        designToggleBtn.setAttribute("aria-pressed", String(!!state.showDesign));
      }
    };
    ...
    const updateOverlay = (area) => {
      ...
      overlayRect.style.display = state.showFrame ? "" : "none";
      const hasImage =
        previewImg.complete &&
      ...
      const shouldShowDesign = state.showDesign;
      designOverlay.style.display = shouldShowDesign ? "" : "none";

      const stage = previewImg.closest(".product-preview__stage");
      if (!stage) return;
      ...
      designSvg.setAttribute("width", shouldShowDesign ? rectW : 0);
      designSvg.setAttribute("height", shouldShowDesign ? rectH : 0);
