(function () {
  ...
  window.PanelModules["canvas-settings-panel"] = function initCanvasSettingsPanel(root) {
    ...
    const activeCanvasToggle = root.querySelector("[data-active-canvas]");
    const showRegionToggle = root.querySelector("[data-show-region]");
    ...
    const frameToggleInputs = [showRegionToggle].filter(Boolean);
    ...
    const syncSideEnableToggle = () => {
      if (!activeCanvasToggle) return;
      const enabled = isSideEnabled(state.side);
      activeCanvasToggle.checked = enabled;
      activeCanvasToggle.setAttribute("aria-pressed", String(enabled));
    };
    ...
    const renderSideTabs = () => {
      ...
      sides.forEach((side) => {
        ...
        const enabled = isSideEnabled(side.id);
        btn.type = "button";
        ...
        btn.innerHTML = `
          <input type="checkbox" data-tab-enable value="${side.id}" ${enabled ? "checked" : ""} aria-label="Use ${side.label}" />
          <span>${side.label}</span>
        `;
        const checkbox = btn.querySelector("[data-tab-enable]");
        checkbox?.addEventListener("click", (event) => event.stopPropagation());
        checkbox?.addEventListener("change", (event) => {
          const next = Boolean(event.target.checked);
          state.enabledSides[side.id] = next;
          if (state.side === side.id) {
            render();
          } else {
            renderSideTabs();
          }
        ...
        btn.addEventListener("click", (event) => {
          if (event.target.closest("[data-tab-enable]")) return;
    ...
    activeCanvasToggle?.addEventListener("change", () => {
      state.enabledSides[state.side] = Boolean(activeCanvasToggle.checked);
      render();
    });
