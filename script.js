...
function applyOffset(axis, rawValue) {
  ...
  // allow full declared slider range without clamping to visible area
  const maxOffset = 1000;
  const offset = Math.max(-maxOffset, Math.min(maxOffset, rawValue));

  // get reference center
  const axisCenter = axis === 'x' ? center.x : center.y;

  // move freely: no design-area clamp
  const absolute = axisCenter + offset;
  const finalOffset = Math.round(offset);

  if (axis === 'x') {
    layer.cx = absolute;
    // do NOT overwrite slider value — prevents visible capping
    if (preciseX) preciseX.value = finalOffset;
  } else {
    layer.cy = absolute;
    if (preciseY) preciseY.value = finalOffset;
  }
